...
# 검사기법
경계값 분석 1.동치 분할 검사 + 2.원인 효과 그래프 검사 + 3. 비교 검사 = 블랙박스 테스트
기초 경로검사, 조건 검사, 루프 검사, 데이터 흐름검사 = 화이트박스 테스트

## 화이트박스 테스트
- 모듈이 원시 코드를 오픈시킨 상태에서 원시 코드의 논리적인 모든 경로를 검사, 검사 사례를 설계하는 방법
- 종류 : 기초 경로 조사, 조건 검사, 루프 검사, 데이터 흐름 검사
...
모듈단위로 단계적으로 통합하면서 검사하는 방법인 점진적 통합 방식의 하나.
상위 모듈에서 하위 모듈 방향.
주요 제어 모듈이 검사를 위한 드라이버로 사용된다. 종속 모듈은 시험용 모듈인 스터브(Stub)로 대체한다.
이 후 종속 스터브들을 통합하며 검사 실시.
일반적으로 스터브(stub)를 드라이버(driver)보다 쉽게 작성할 수 있다.
보증을 위해 회귀 검사를 실시한다. (역순으로 검사)
상위층에서 검사 사례(test case)를 쓰기가 어렵다.
하향식 통합 검사는 상위 모듈에서 단계적인 통합을 통해 하위 모듈로 내려가며 검사하는 방식입니다.
이렇게 하위로 내려갈땐 반드시 시스템의 구조를 알고 있어야 검사가 수월합니다.
때문에 하향식 통합 검사는 전체적인 시스템의 구조를 파악하고 검사하기 때문에 검사 초기부터 사용자에게 시스템의 구조를 보여줄 수 있습니다.

# 상향식 통합 검사는 Bottom up integratio Test
하위에서 상위로.
하위 모듈을 클러스터로 결합함.
